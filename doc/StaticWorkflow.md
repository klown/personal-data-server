...
4. Making Authorized Requests to the Preferences Server
   1. UIO make a local request to the Preferences Server handled by the Edge Proxy,
      passing the `session_key` in the `Authorization` header.
   2. The Edge Proxy redirects this request to a Preferences Server
      end point
   3. The Preferences Server validates the `session_key` authorization.
...
**3i**. The Preferences Server generates a time-limited `session_key` that is associated
with the authenticated Resource Owner.

This session key is persisted within the Preferences Server's database and associated
to the Resource Owner's `SsoAccount`.  This `session_key` has a expiration that is tied
to the `access_token` generated by the SSO provider at the end of the Single Sign On
workflow (step 2vii).  That `access_token` has an expiration date as well, that is set
by the SSO provider.  The `session_key` generated at the current step expires at the same
time.  When the SSO `access_token` expires and is refreshed, the `session_key` is
simultaneously refreshed.

**3ii, 3iii**. The Preferences Server responds to the Edge Proxy's `/authenticate` request
sending the session key to the Edge Proxy.  Similarly, the Edge Proxy responds to UIO's
...
{
    "session_key":"2YotnFZFEjr1zCsicMWpAA",
    "token_type":"bearer",
}
```
...
| Name             | Type     | Description |
| ---              | ---      | ---         |
| `session_key`    | `String` | __Required__. Generated by the Preferences Server to use to access the Resource Owner's preferences on the Preferences Server |
...
| Name             | Type     | Description |
| ---              | ---      | ---         |
| `session_key`    | `String` | __Required__. The same value generated and returned by the Preferences Server at step 3ii and 3iii |
...
**4iii**. The Preferences Server validates the `session_key` authorization.

The Preferences Server compares the given `session_key` against the one(s) in its
...
communicates with the SSO provider.  The client also specifies a redirect uri for
...
client.  These are included in the provider's login and consent dialogs to inform
users which client is requesting access to the user's information.  The links
...
parameters are taken from the [OAuth2 specification](https://tools.ietf.org/html/rfc6749#section-4.1.1)
...
* The `access_token` can be added to the `Authorization` header of requests sent
  to Google by the Preferences Server.
* Reference [OAuth2 Access Token Response](https://tools.ietf.org/html/rfc6749#section-4.1.4)

## To Do

* Investigate access token expiration and refresh tokens and how to use that to refresh the
  authorization session key passed between the Preferences Server/Edge Proxy and UIO
