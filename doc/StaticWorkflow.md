...
**1i**. Resource Owner interacts UIO's user interface to log into the Preferences
Server using the Google credentials.
...
| ---     | ---      | ---         |
| `sso`   | `String` | __Required__. Identfier of the SSO provider to use, e.g. `google` |

Payload: None

This triggers the SSO workflows which is fully described in the [Single Sign On Details](#single-sign-on-details)
section.
...
time.  When the SSO `access_token` expires and is refreshed, the `session_key` will also be refreshed.
...
The Preferences Server compares the given `session_key` against the `SsoAccount` records
in its database.  If it finds match, it cross references that record against the `User`
...
* Investigate access token expirataion and refresh tokens and how to use that to refresh the
  authorization session key passed beween the Preferences Server/Edge Proxy and UIO
