...
`AppSsoProvider` records are primarily for storing the `client_id` and `client_secret` generated by the SSO provider during registration.  The client_id uniquely identifies the Preferences Server.  Information about the provider itself is also stored here.
...
Records that contains information about users.  This is based on [`fluid-express-user`](https://github.com/fluid-project/fluid-express-user/blob/master/tests/data/users.json).  The values for a number of these fields are given by the SSO provider as part of the response to the user's successfully signing into that provider.  These are `name`, `username`, `email`, and `verified` (email is verified). Refer to [fluid-express-user data](https://github.com/fluid-project/fluid-express-user/blob/master/tests/data/users.json) for
...
These records store the information about the user as returned by the SSO provider.  As such, they cross reference `User` and `SsoProvider` records.
...
The access token returned by the SSO Provider that is used for access to the user's information stored by the Provider.  These records also store the login token that the Preferences Server passes back to the Edge Proxy and Resource owner for accessing the user's preferences.  The records cross references the user's `SsoAccount` and `AppSsoProvider`.
